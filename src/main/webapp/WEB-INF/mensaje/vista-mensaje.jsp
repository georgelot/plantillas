...
<body>
	<div class="container">
		<div class="row">
			<div class="col-12">
				<form action='<c:url value="/mensaje/ingresar" />' modelAttribute="mensaje" method="post">
		...
		<c:forEach var="mensaje" items="${mensajes}">
			<div class="row my-2">
				<div class="col-12">
					<div class="card">
						<div class="card-body">
							<h5 class="card-title"><c:out value="${mensaje.getId()}" /> - <c:out value="${mensaje.getRemitente()}" /></h5>
							<p class="card-text"><c:out value="${mensaje.getMensaje()}"></c:out></p>
							<a href='<c:url value="/mensaje/eliminar?id=${mensaje.getId()}" />' class="btn btn-warning">Eliminar</a>
						</div>
					</div>
				</div>
			</div>
